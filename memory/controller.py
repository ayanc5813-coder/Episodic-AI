...
class MemoryController:
    ...
    def event_by_tag(self, key, tag, note):
        ...
        if len(event_ids) > config.RERANK_LIMIT:
            ...
            # [fix] after dropping the evidence param, the return value was missed: evidence is the retrieved event_ids (matches official/v8/v666)
        ...
        if pattern.match(event_id):
            for i in range(1,10):
                if f"{event_id}-{i}" in self.memory.episode_events:
                    ...
                    # [LM] for LM, collect all variants of an event (skip numbering gaps) -> richer keywords/tags for navigation; locomo keeps break (matches v666)
    ...
    def evaluate_relations_over_graph(
        ...
        # [LM] from official: in LM all events hang under "user"; force-add a "user" query key to recall all user-related events
        if config.dataset == "LM":
            if "user" not in qmap:
