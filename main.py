...
def get_question(dataset, agent, question_list, sample_id, memory, result_path, question_embeddings=None):
    ...
    def _run_one_question(i, qa):
        ...
        # [F-LM] verified: in v666 (the 72% run) the answer input contains current_date=question_date (e.g. 2023-04-01).
        # but do not use override_question_time (it pushes the question onto the "narrow time path" and loses evidence).
        # approach: keep question_time empty -> main path + navigation; separately inject question_date as the current_date anchor on the main-path ans_input.
...
def main():
    ...
    i=0
    # [LM] for LM, filter samples by category (from v8)
    ...
    for sample_id, sample in conversation_list.items():
        ...
        if dataset == "LM":
            ...
            if cat != category_dict[config.ca]:
                continue
        else:
