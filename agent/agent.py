...
class Agent:
    ...
    def answer_question_with_time_lm(self, question, question_emb, query_answers, question_time, question_keys, category):
        # [F] LM-specific temporal handling: ported verbatim from reasoning_v8's answer_question_with_time
        events_time = self.memory_controller.get_time_event(question_time)
        events_time_originlist = list()
        for time in events_time:
            events_time_originlist.append(time.split("-")[0].strip())
        if len(events_time) < config.K2:
            ...
            ans_input = {
                ...
                "current_date": question_time.split(",")[0].strip() if question_time else None,  # current date (the "today" at question time), from official
            }
        ...
        if len(coarse_ids) > config.K2:
            ...
            if question_out is None:
                ...
                if sort_ids != None:
                    for id in sort_ids:
                        match_idx = None
                        # [LM] from official: LM ids may carry a speaker suffix (e.g. D27:34-1:Joanna); match tolerantly by stripping the speaker
    ...
    def answer_question(self, question: str, category=0, question_emb=None, override_question_time=None, lm_current_date=None) -> Dict[str, Any]:
        ...
        query_answers = self.memory_controller.evaluate_relations_over_graph(question_keys.get("keywords"))
        # [F-LM] if an override is given (LM temporal question_date anchor) use it; otherwise use the LLM-extracted time (from official)
        ...
        if question_time:
            if config.dataset == "LM":
                # [F] LM uses v8's temporal handling
                ...
                # [F] locomo uses copy's original temporal handling
        ...
        if (not _lm_time_done) and ((not question_time) or len(ans_input.get("similar_sentence", [])) == 0):
            ...
            # [F-LM] inject current_date (=question_date) on the main path as the "now" anchor for temporal questions (matches v666 gemini_time2)
            if lm_current_date:
        ...
        if category == 2:
            ...
            # [E] from v8: open-ended questions are asked to give supporting reasons
            ans_input["question"] = ans_input["question"] + (" No extra explanations in 'answer'. Give reasons with original text in 'reason'. ")
        # [LM] LM uses official's ANSWER system prompt (how-many/temporal rules + strong tool navigation); locomo uses copy's. how-many/temporal is already in that prompt, so it is not appended to the question
    ...
    def extract_keyword(self, keyword_path: str, rewrite_path:str, ref_id:int):
        ...
        with open(file_name, "a", encoding="utf-8") as f:
            for i in range(len(record_rewrite)):
                if i < ref_id:
                    continue
                # [robust read, matches v666] a rewrite record key may be session_i (batch=1) or session_first-session_last (batch>1),
                ...
                if session_data is None:
                    ...
                    f.write("null\n")
                    continue
                # [matches official/v666] feed only {id, text} to extract_keys, dropping the rewrite-stage tag/origin/topic/time,
                # to avoid leaking existing tag/topic into the keyword-extraction prompt (else the LLM copies them, diverging from official).
    ...
    def store_keyword(self, keyword_path: str, rewrite_path:str,) -> None:
        ...
        for i in range(len(records_key)):
            # [robust read, matches v666] a rewrite record key may be session_i or session_first-session_last,
